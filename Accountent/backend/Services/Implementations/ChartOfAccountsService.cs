...
namespace backend.Services.Implementations
{
    public class ChartOfAccountsService : ServiceBase<ChartOfAccountsService>, IChartOfAccountsService
    {
        ...
        public async Task<ServiceResult<AccountResponse>> CreateAsync(CreateAccountRequest request)
        {
        ...
        public async Task<ServiceResult<AccountResponse>> UpdateAsync(int id, UpdateAccountRequest request)
        {
            ...
            if (request.parent_id == id)
                return ServiceResult<AccountResponse>.Fail(
                    ...
                    "Счёт не может быть родителем самого себя.");
            ...
            if (request.parent_id is not null && await IsDescendantAsync(db, request.parent_id.Value, id))
                return ServiceResult<AccountResponse>.Fail(
                    ServiceErrorCode.Validation,
                    "Нельзя назначить дочерний счёт родителем.");
        ...
        public async Task<ServiceResult> DeleteAsync(int id)
        {
            ...
            if (account is null)
                return ServiceResult.Fail(ServiceErrorCode.NotFound, "Счёт не найден.");

            if (account.is_system)
                return ServiceResult.Fail(
                    ServiceErrorCode.Forbidden,
                    "Системный счёт из типового плана нельзя удалить.");

            if (account.children.Any(c => !c.deleted))
                return ServiceResult.Fail(
                    ServiceErrorCode.Conflict,
                    "Нельзя удалить счёт, у которого есть дочерние счета.");
        ...
        private static async Task<(Account? parent, ServiceError? error)> ResolveParentAsync(
            Data.ApplicationDbContext db, int? parentId, int? excludeId = null)
        {
            ...
            if (parent is null)
                return (null, ServiceErrors.NotFound("Родительский счёт не найден."));

            if (excludeId is not null && parent.id == excludeId)
                return (null, ServiceErrors.Validation("Некорректный родительский счёт."));
